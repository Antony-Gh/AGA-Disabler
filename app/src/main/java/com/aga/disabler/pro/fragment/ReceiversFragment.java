...
public class ReceiversFragment extends FragmentHolder{
    private ActivityInfo[] activityInfos;
    private PackageManager pkgmg;
    private ApplicationPolicy ap;

    public static ReceiversFragment getInstance(Context context, String pkgname) {
        ReceiversFragment fragment = new ReceiversFragment();
        fragment.setContext(context);
        fragment.setTitle("Receivers");
    ...
    public void init() {
        ...
        activityInfos = getarchiveinfo(c, pkg, PackageManager.GET_RECEIVERS).receivers;
        if(activityInfos != null && activityInfos.length != 0) {
            act = Arrays.asList(activityInfos);
        }
        if(act.isEmpty()){
            TextView t = view.findViewById(R.id.textview1);
            t.setText(t.getText().toString().replace("%d", "Receivers"));
